    ...
    //  && : If both are true it returns true (AND)
    //  || : If either are true it returns true (OR)
    //  ! : Converts true into false and vice versa (NOT)
 
    // --- EXAMPLE : IS A BIRTHDAY IMPORTANT OR NOT---Impt only if aged b/n : 1 - 18, 21, 50, > 65    
        std::string sAge = "0";
        std::cout << "Enter your age : ";
        getline(std::cin, sAge);
        int nAge = std::stoi(sAge);

        // if and else is used to execute different code depending on conditions
        if ((nAge >= 1) && (nAge <= 18))
            {
